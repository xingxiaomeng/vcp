...
(function () {
    ...
    var MUSIC_HTML = [
        ...
        '            }',
        '            ',
        '            var pos = state.current_time ?? state.position_secs ?? state.position ?? 0;',
        '            var dur = state.duration ?? state.duration_secs ?? 0;',
        ...
        '        try {',
        '            var state = await musicAPI.getState();',
        '            if (state && (state.duration ?? state.duration_secs ?? 0) > 0) {',
        '                var rect = progressBar.getBoundingClientRect();',
        '                var ratio = (e.clientX - rect.left) / rect.width;',
        '                var totalDur = state.duration ?? state.duration_secs ?? 0;',
        '                var seekPos = ratio * totalDur;',
