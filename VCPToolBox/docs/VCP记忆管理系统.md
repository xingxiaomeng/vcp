...
- **未来的基石**: 这一坚实的底层架构，为未来 `RagTagMemo` 系统的进一步深入优化和功能拓展打下了不可撼动的基石，确保了 VCP 记忆系统长期、高效、稳定地进化。
	...
	*   **Rust驱动的内存安全与极致性能**: 核心逻辑由Rust构建，引入了原创的语义级Chunk去重函数，杜绝内存安全问题，确保系统长期稳定并完全释放硬件性能。
	...
	-   **通用 BM25 / BM25+ 召回后缀**：
	    ...
	    -   在 `[[]]`、`《《》》` 与知识库语法中，`::BM25` / `::BM25+` 会作为 RAG 管线的稀疏召回分支参与候选池构建，再继续与向量检索、TagMemo、Rerank、Truncate、Expand、Associate、AIMemo+ 等后续能力融合；其中 `::BM25` 偏向标签/结构化关键词，`::BM25+` 偏向正文稀疏匹配。
	    -   在 `<<>>` 语法中，`::BM25` / `::BM25+` 仍先经过相似度门控；门控通过后复用 `{{}}` 纯文本管线，注入命中文件全文。
	    -   `::BM25` 与 `::BM25+` 均可和 `::LastN` 配合限定候选范围，例如 `{{小雨日记本::BM25::Last30}}`、`<<小雨日记本::BM25+::Last50>>`、`[[小雨日记本::BM25::Last30::Rerank]]` 或 `《《VCP开发文档知识库::BM25+::Truncate0.4》》`；如果只写 `::BM25` / `::BM25+` 而不写 `::LastN`，默认只在最新 10 篇日记/候选文档中检索。
	...
	-   **高级用法**：此模式支持强大的 **时间感知检索**、**语义组增强检索** 与通用 **BM25 / BM25+ 稀疏召回**，详见“高级功能与配置”部分。示例：`[[角色日记本::BM25::Rerank]]`、`[[角色日记本::BM25+::TagMemo+::Rerank+]]`。
	...
	-   **与 `[[]]` / `《《》》` 的区别**：`<<>>` 触发后注入的是日记文件全文或 BM25 命中文件全文；`[[]]` 与 `《《》》` 走 RAG 片段检索管线，可叠加 TagMemo、Rerank、Time、Expand、Associate、AIMemo+ 等语义检索能力。其中 `[[角色日记本::BM25]]` / `[[角色日记本::BM25+]]` 会直接进入 RAG 片段管线并混入 BM25 稀疏候选；`《《角色日记本::BM25》》` / `《《角色日记本::BM25+》》` 是“门控后进入 RAG 片段管线并混入 BM25 稀疏候选”，而不是 `<<>>` 这种纯文本全文注入。
	...
	**Base64Memo** 为日记本检索引入了跨模态感知能力。它允许系统自动从召回的日记正文中提取多媒体附件，并将其以 Base64 格式注入到当前对话中。
	...
	-   **修饰符兼容建议**：
	    -   推荐组合：`::BM25`、`::BM25+`、`::Rerank`、`::Rerank+`、`::Truncate{阈值}`、`:K倍率`，用于提高冷知识库召回精度。
	    -   `::BM25` / `::BM25+` 在知识库语法中同样是通用稀疏召回修饰符，可与向量检索结果共同进入候选池；`::BM25` 更适合标签、标题、结构化关键词命中，`::BM25+` 更适合正文关键词补召回。示例：`[[项目接口知识库::BM25+::Rerank+]]`、`《《VCP开发文档知识库::BM25::Truncate0.4》》`。
